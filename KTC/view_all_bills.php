...
if(mysqli_num_rows($result_importer22) > 0)
{
    ...
    {
        echo '&nbsp'.$row22['customer_name'].',<br>&nbsp'.$row22['customer_address'].',<br>&nbsp'.$row22['customer_mobile_no'].',<br>&nbspGST/PAN/Adhar No.:'.$row22['customer_GST_NO'];
    }
}
...
<td width="350">
<b>&nbspInvoice No.:- '.$_POST["invoice_number"].'/19-20 <br>
...
$query_importer = "SELECT sum(taxable_value) as allTotal FROM bills where invoice_number='".$_POST["invoice_number"]."'";
      $result_importer = mysqli_query($conn,$query_importer);
if(mysqli_num_rows($result_importer) > 0)
{
    while($row = mysqli_fetch_assoc($result_importer))
...
$query_importer = "SELECT sum(cgst_amount) as allTotal FROM bills where invoice_number='".$_POST["invoice_number"]."'";
      $result_importer = mysqli_query($conn,$query_importer);
if(mysqli_num_rows($result_importer) > 0)
{
    while($row = mysqli_fetch_assoc($result_importer))
...
$query_importer = "SELECT sum(sgst_amount) as allTotal FROM bills where invoice_number='".$_POST["invoice_number"]."'";
      $result_importer = mysqli_query($conn,$query_importer);
if(mysqli_num_rows($result_importer) > 0)
{
    while($row = mysqli_fetch_assoc($result_importer))
