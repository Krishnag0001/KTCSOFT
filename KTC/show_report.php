...
$query_importer5 = "SELECT sum(cgst_amount) as allTotal FROM bills where invoice_number='".$row["invoice_number"]."'";
      $result_importer5 = mysqli_query($conn,$query_importer5);
if(mysqli_num_rows($result_importer5) > 0)
{
    while($row5 = mysqli_fetch_assoc($result_importer5))
    {
    echo $row5["allTotal"];
    }
...
echo '</td>
<td style="text-align:right">';
$query_importer6 = "SELECT sum(sgst_amount) as allTotal FROM bills where invoice_number='".$row["invoice_number"]."'";
      $result_importer6 = mysqli_query($conn,$query_importer6);
if(mysqli_num_rows($result_importer6) > 0)
{
    while($row6 = mysqli_fetch_assoc($result_importer6))
    {
    echo $row6["allTotal"];
    }
...
<td style="text-align:right"> 
';
$query_importer = "SELECT sum(cgst_amount) as allTotal FROM bills where status=1 and created_date between '".$date1."' and '".$date2."'";
      $result_importer = mysqli_query($conn,$query_importer);
      if(mysqli_num_rows($result_importer) > 0)
{
    while($row = mysqli_fetch_assoc($result_importer))
...
<td style="text-align:right">
';
$query_importer = "SELECT sum(sgst_amount) as allTotal FROM bills where status=1 and created_date between '".$date1."' and '".$date2."'";
      $result_importer = mysqli_query($conn,$query_importer);
      if(mysqli_num_rows($result_importer) > 0)
{
    while($row = mysqli_fetch_assoc($result_importer))
