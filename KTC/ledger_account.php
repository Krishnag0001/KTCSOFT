...
function DeleteLedger(id_invoice){

   var r = confirm("Do you really want to delete?");
  if (r == true) {
    

    window.location="sql_delete_ledger.php?id="+id_invoice;
    //txt = "You pressed OK!";
  } 
  else 
  {
...
include 'config.php';
                    echo '
                            ...
                            <h4 class="box-title">Ledger Account</h3>
                            
<b>Account Name:-</b> ';

    $query_importer22 = "SELECT * FROM customer where id_customer='".$_POST['name']."'";
...
if(mysqli_num_rows($result_importer22) > 0)
{
    ...
    {
        echo $row22['customer_name'].', '.$row22['customer_address'].', Mo.No: '.$row22['customer_mobile_no'].', GST/PAN/Adhar: '.$row22['customer_GST_NO'];
...
$query_importer = "SELECT * FROM ledger_account where customer_id='".$_POST['name']."'";
      $result_importer = mysqli_query($conn,$query_importer);
...
';
        ...
        <td style="text-align:center">'.$row['invoice_id'].'</td>
        <td style="text-align:center">'.$row['debit_amount'].'</td>
        <td style="text-align:center">'.$row['credit_amount'].'</td>
<td style="text-align:center">';

    echo '<b><a href="javascript:DeleteLedger('.$row["id_ledger_account"].');"><i class="glyphicon glyphicon-remove"></i></a></b>';
...
<td style="text-align:center">
';
$query_importer = "SELECT sum(debit_amount) as debitTotal FROM ledger_account where customer_id='".$_POST['name']."'";
      $result_importer = mysqli_query($conn,$query_importer);
if(mysqli_num_rows($result_importer) > 0)
{
    while($row = mysqli_fetch_assoc($result_importer))
...
$query_importer = "SELECT sum(credit_amount) as creditTotal FROM ledger_account where customer_id='".$_POST['name']."'";
      $result_importer = mysqli_query($conn,$query_importer);
if(mysqli_num_rows($result_importer) > 0)
{
    while($row = mysqli_fetch_assoc($result_importer))
...
$query_importer = "SELECT sum(debit_amount) as debitTotal,sum(credit_amount) as creditTotal FROM ledger_account where customer_id='".$_POST['name']."'";
      $result_importer = mysqli_query($conn,$query_importer);
if(mysqli_num_rows($result_importer) > 0)
{
    while($row = mysqli_fetch_assoc($result_importer))
